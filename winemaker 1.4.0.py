...
while True: #run loop 
    ...
    while len(all_inv_jug_window) < 13:
        ...
        if jug_click_wait > 7:
            print('waited %s, not seeing enough jugs. dumping inventory and retrying grape and jug with no safetys this time...' % len(jug_click_wait))
            bank_grape.genclick()
            time.sleep(.3 + abs(np.random.normal(0,.3)))
            bank_jug.genclick()

        if jug_click_wait > 20:
    ...
    while len(all_inv_grape_window) > 0:
        ...
        if grapewatch_wait > 60:
            print('PROBLEM(!) grapewatch_wait = %s. giving up, exiting...' % grapewatch_wait)
            exit()
